Special Instruction: HTMLのサイズはどれほど肥大化してもいいので、全力を惜しまず出してみて
...
0. 今回のインフォグラフィックの「見せ方の方向性」を1つ決める。
   * 例: 報告書寄りミニマル、ポスター寄り、大きな図版中心、ダークテーマ高密度、余白多めなど。
   * 想定読者・利用シーン（画面閲覧、印刷、プレゼン投影など）と整合しているかを確認し、途中でぶらさない。

1. 親デザイン案を3〜5系統作る。
   * 構図（1カラム/2カラム/カード型など）、情報密度（スカスカ〜高密度）、色調（ライト/ダーク/中庸）で互いに距離がある案にする。
   * 各案について、見出し・本文・注記のタイポ階層、グリッド、図版の配置方針を箇条書きレベルで内部整理する。

2. 各案を、可読性・視線誘導・アクセシビリティ・印刷時の判読性の観点で評価する。
   * 見てほしい要素に自然に視線が流れるか。
   * コントラストが十分か（おおよそ4.5:1目安）。
   * 情報量に対してレイアウトが破綻していないか。

3. 評価結果にもとづき、上位1〜2案をエリート保存しつつ、要素を組み替えたハイブリッド案を1〜2回生成する。
   * 配色セット／グリッド／図版配置をバラバラにせず、依存関係を保ったまま交叉・微修正するイメージで洗練させる。
   * 「AIがよくやるテンプレUI」（なんとなくのグラデーション、3カード横並びだけ、根拠のない装飾）に収束していないかをチェックし、そうなっていたら構図か配色を意図的に変える。

4. 最終案を1つ選び、インフォグラフィック全体のスタイルとして固定する。
   * ベースカラー1色＋アクセント1〜2色、最大5色以内に収め、同じ意味には同じ色を使う。
   * フォント種別はプロンプト指定のスタック内で最大3ロール（大見出し/小見出し/本文＋注記）に整理し、サイズ・太さ・余白で階層を明確にする。

5. 最終案にもとづき主SVGを設計する。
   * viewBox必須、左右2〜4%ガター、half stroke幅、marker長・filter境界・ラベルのはみ出しに余裕を持たせる。
   * vector-effect="non-scaling-stroke" を必要箇所に付与し、data-convert-png="true" と一意の id を全ての主SVGに付ける。
   * <title><desc> で図の意味を文章化する。

6. CSSとSVGが同じ「デザインの意図」を表現しているかを確認する。
   * 見出し階層・色使い・余白・罫線スタイルが本文と図版の間で矛盾していないか。
   * 図版を含む全体を眺めたときに、「このインフォグラフィックならではの雰囲気」が一貫しているかを工程Cの検査として確認する。
   * PNG変換に必要な属性（id, data-convert-png など）が全主SVGに付いていることを確認し、結果をchecksおよびartifactsに反映する。

終了時:

* wip.svg_modules / assembly_map を更新する。
* artifacts.C_visual_synthesis に検討したデザイン案と評価を追記する。
* PNG変換を考慮した属性がすべての主SVGに付与されていることを確認し、結果をchecksおよびartifactsに反映する。
